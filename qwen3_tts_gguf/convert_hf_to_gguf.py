...
class ModelBase:
    ...
    def load_hparams(dir_model: Path, is_mistral_format: bool):
        ...
        # Force load from config.json first to support spoofing architectures
        if (dir_model / "config.json").is_file():
            try:
                with open(dir_model / "config.json", "r", encoding="utf-8") as f:
                    config = json.load(f)
                    logger.info(f"Loaded config from {dir_model / 'config.json'}")
                    return config
...
class Qwen3VLTextModel(Qwen3Model):
    ...
    def set_gguf_parameters(self):
        ...
        # deepstack_visual_indexes might be in vision_config or root
        deepstack_visual_indexes = vision_config.get("deepstack_visual_indexes", self.hparams.get("deepstack_visual_indexes", []))
        deepstack_layer_num = len(deepstack_visual_indexes)
        
        self.gguf_writer.add_num_deepstack_layers(deepstack_layer_num)
        
        # Add rope dimension sections if present (critical for Qwen3-TTS)
        rope_scaling = self.hparams.get("rope_scaling", {})
        if rope_scaling is None: rope_scaling = {} # Handle None
        if (mrope_section := rope_scaling.get("mrope_section")) is not None:
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # QK-Norm support for Qwen3-TTS
        if "q_norm.weight" in name and bid is not None:
            # Manual mapping: blk.{bid}.attn_q_norm.weight
            return [(f"blk.{bid}.attn_q_norm.weight", data_torch)]
        if "k_norm.weight" in name and bid is not None:
            return [(f"blk.{bid}.attn_k_norm.weight", data_torch)]
